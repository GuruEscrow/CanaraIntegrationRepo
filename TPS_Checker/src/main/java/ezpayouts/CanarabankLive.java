...
public class CanarabankLive {
	...
	// MzI1MjczNzkxQVBJOjRhOGE0OGEyNGE0MWEwYTk0YTg3YTgzYTExMg==\",\"acctNumber\":\"120029248620
	...
	/**
	 * Request (json) payload before encryption private static final String REQ_BODY
	 * = "{" + "\"Request\": " + "{ \"body\": {" + "\"branchCode\": \"5573\"," +
	 * "\"encryptData\": \"%s\"" + "}" + "}" + "}";
	 */
	...
	public Map<String, String> getStatement(String fromDate, String toDate) throws Exception {
		...
		if (responseObj.get("Response") != null) {
			...
			// Updating the statements into Map with key UTR
			JsonArray stmtArray = JsonParser.parseString(statementsStr).getAsJsonObject()
					.getAsJsonArray("transactions");
			for (int txn = 0; txn < stmtArray.size(); txn++) {
				JsonObject individualTxn = stmtArray.get(txn).getAsJsonObject();
				String utr = null;
				if (individualTxn.has("chequeNumber")&&individualTxn.has("txnRefNumber")) {
					utr = individualTxn.get("chequeNumber").getAsString().trim();
				}else {
					utr = individualTxn.get("txnRefNumber").getAsString().trim();
				}
				
				stmtMap.put(utr, individualTxn.toString());
			}
		} else {
			System.err.println(responseObj);
		}
	...
	public Map<String, String> neftTransfer(String beneName, String beneAccNo, String beneIFSC, String amount, String transactionNote,
			String valueDate) throws Exception {
		...
		// Storing Pay load plain with user_ref to DataBase
		Map<String, String> payoutDetails = new HashMap<String, String>();
		JsonObject payloadJson = JsonParser.parseString(PAY_LOAD_PLAIN).getAsJsonObject();
		String user_ref = payloadJson.getAsJsonObject("Request").getAsJsonObject("body").getAsJsonObject("encryptData")
				.get("userRefNo").getAsString();
		InsertDataIntoDB.insertPayloadWithUserRef(user_ref, PAY_LOAD_PLAIN, "neft");
		payoutDetails.put("user_ref", user_ref);
		...
		try {
			JsonObject responseObj = JsonParser.parseString(response).getAsJsonObject();
			if (responseObj.get("Response") != null) {
				String encr = responseObj.getAsJsonObject("Response").getAsJsonObject("body").get("encryptData")
						.toString();
				System.out.println(decrypt(encr.replaceAll("\"", "")));
				
				// Storing the txn response with UTR to DB
				String txnResposne = decrypt(encr.replaceAll("\"", ""));
				String utr = JsonParser.parseString(txnResposne).getAsJsonObject().getAsJsonObject("txnMessages")
						.get("transactionRefNo").getAsString();
				payoutDetails.put("utr", utr);
				InsertDataIntoDB.updateTxnResponse(utr, txnResposne, user_ref);

			} else {
				System.err.println(responseObj);
				
				// Storing the txn response with UTR to DB
				payoutDetails.put("utr", "null");
				InsertDataIntoDB.updateTxnResponse("null", responseObj.getAsString(), user_ref);
			}
		} catch (Exception ex) {
			...
			// Storing the txn response with UTR to DB
			payoutDetails.put("utr", "null");
			InsertDataIntoDB.updateTxnResponse("null", "Error: " + response, user_ref);
	...
	public void neftStatus(String userRefNo) throws Exception {
		...
		try {
			...
			if (responseObj.get("Response") != null) {
				...
				// Updating the txn status to DB
				String decryptedData = decrypt(encr.replaceAll("\"", ""));
				InsertDataIntoDB.updateTxnStatus(responseObj.toString() + " " + decryptedData, userRefNo,null);
			} else {
				System.err.println(responseObj);
				
				// Updating the txn status to DB
				InsertDataIntoDB.updateTxnStatus("Response Null: " + responseObj.toString(), userRefNo,null);
			}
		} catch (Exception ex) {
			System.out.println(response);
			
			// Updating the txn status to DB
			InsertDataIntoDB.updateTxnStatus("Error: " + response, userRefNo,null);
	...
	public void rtgsTransfer(String beneName, String beneAccNo, String beneIFSC, String amount, String transactionNote,
			String valueDate) throws Exception {
		String userRefNo = "PHED" + System.currentTimeMillis();
		String DATA_TO_ENCRYPT = "{\n"
				+ "                \"Authorization\": \"Basic MzI1MjczNzkxQVBJOjRhOGE0OGEyNGE0MWEwYTk0YTg3YTgzYTExMg==\",\n"
	...
	public Map<String, String> impsTransfer(String beneName, String beneAccNo, String beneIFSC, String amount,
			String transactionNote) throws Exception {
		String userRefNo = "PHED" + System.currentTimeMillis();
		String DATA_TO_ENCRYPT = "{\n"
				+ "                \"Authorization\": \"Basic MzI1MjczNzkxQVBJOjRhOGE0OGEyNGE0MWEwYTk0YTg3YTgzYTExMg==\",\n"
				+ "                \"txnPassword\": \"4a11a44a2a49a0a94a87a83a115\",\n"
		...
		String user_ref = payloadJson.getAsJsonObject("Request").getAsJsonObject("body").getAsJsonObject("encryptData")
				.get("userRefNo").getAsString();
		InsertDataIntoDB.insertPayloadWithUserRef(user_ref, PAY_LOAD_PLAIN, "imps");
		payoutDetails.put("user_ref", user_ref);
		...
		try {
			response = invokeUniRequest(url, payload, sign);
		}catch(Exception e) {
			// Storing the txn response with UTR to DB
            payoutDetails.put("utr", "null");
			InsertDataIntoDB.updateTxnResponse("null", "Txn not initiated: " + e.getMessage(), user_ref);
			return null;
		}
		...
		try {
			...
			if (responseObj.get("Response") != null) {
				...
				// Storing the txn response with UTR to DB
				payoutDetails.put("utr", "null");
				InsertDataIntoDB.updateTxnResponse("null", "False:" +responseObj.getAsString(), user_ref);
			}
		} catch (Exception ex) {
			System.out.println(response);
	...
	public void impsStatus(String userRefNo) throws Exception {
		...
		try {
			...
			if (responseObj.get("Response") != null) {
				...
				// Updating the txn status to DB
				String decryptedData = decrypt(encr.replaceAll("\"", ""));
				JsonObject statusBody = responseObj.getAsJsonObject("Response").getAsJsonObject("body");
				String status = null;
				if(statusBody.has("status")) {
					status = statusBody.get("status").getAsString();
				}else {
					//Let it be null
				}
				InsertDataIntoDB.updateTxnStatus(responseObj.toString() + " " + decryptedData, userRefNo,status);
			} else {
				System.err.println(responseObj);
	...
	public Map<String, String> internalTransfer(String beneName, String beneAccNo, String amount, String transactionNote,
			String valueDate) throws Exception {
		String userRefNo = "PHED" + System.currentTimeMillis();
		String DATA_TO_ENCRYPT = "{\n"
				+ "                \"Authorization\": \"Basic MzI1MjczNzkxQVBJOjRhOGE0OGEyNGE0MWEwYTk0YTg3YTgzYTExMg==\",\n"
		...
		String user_ref = payloadJson.getAsJsonObject("Request").getAsJsonObject("body").getAsJsonObject("encryptData")
				.get("userRefNo").getAsString();
		InsertDataIntoDB.insertPayloadWithUserRef(user_ref, PAY_LOAD_PLAIN, "internal");
		payoutDetails.put("user_ref", user_ref);
...
//	}
	...
	private String invokeUniRequest(String url, String payload, String signature) {
		...
		kong.unirest.HttpResponse<String> response = Unirest.post(url).header("x-client-id", API_KEY)
				...
				.header("x-timestamp", "1675780846").header("x-client-certificate", CLIENT_PUBLIC_CERT)
				...
				.header("X-Forwarded-For", "weasdwd").body(payload).asString();
	...
	public static void main(String[] args) throws NoSuchAlgorithmException, UnsupportedEncodingException, InvalidKeySpecException, JoseException, DecoderException {
		
		CanarabankLive bank = new CanarabankLive();
		String encry = "eyJlbmMiOiJBMTI4Q0JDLUhTMjU2IiwiYWxnIjoiQTI1NktXIn0.H9kGff9DAdPbcEur91_21zPo8oWA_xY8VdM68NSzXbpY0zMY1F4nOw.zLIvyANC9F3D4Ggdd5XHEw.DDfg9Sl7_qRFai9sDq6cnPyuWe127J9PMf3CQP5UN3wAv2i1RowfumrDppoblQ2-7NIh8n8cghpu9CQW4Y1JmWvR4VCYV3qokPftzNf98Ou7b00iej_YCp2Ltaqog6O0QJwq9Sb3Qb_Z-pWvYiK0OQ2eR9tinl5FOrdFiGTTI8LdkDzxgVh4fWxJAq3ZsXpS.tB_9hvbS0hCYN7bwai_hUw";
		
		System.out.println(bank.decrypt(encry));
	}
